...
var author$project$HomePage$emptyErrors = {ip: _List_Nil, name: _List_Nil};
...
var author$project$HomePage$init = function (_n0) {
	return _Utils_Tuple2(
		{errors: author$project$HomePage$emptyErrors, ip: '', name: '', saveRequest: krisajenkins$remotedata$RemoteData$NotAsked},
...
var author$project$HomePage$ipValidator = rtfeldman$elm_validate$Validate$all(
	_List_fromArray(
		[
			A2(rtfeldman$elm_validate$Validate$ifBlank, elm$core$Basics$identity, 'You need to provide an IP address!')
		]));
var author$project$HomePage$ResponseReceived = function (a) {
	return {$: 'ResponseReceived', a: a};
};
var author$project$HomePage$postUrl = 'https://my.api.com/submit';
...
var author$project$HomePage$sendToServer = function (model) {
	return elm$http$Http$post(
		{
			body: elm$http$Http$emptyBody,
			expect: elm$http$Http$expectWhatever(
				A2(elm$core$Basics$composeR, krisajenkins$remotedata$RemoteData$fromResult, author$project$HomePage$ResponseReceived)),
			url: author$project$HomePage$postUrl + ('?' + ('IP=' + (model.ip + ('&' + ('NAME=' + model.name)))))
		});
};
...
var author$project$HomePage$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 'UpdateIp':
				var newIp = msg.a;
				var oldErrors = model.errors;
				var newIpErrors = function () {
					var _n1 = A2(rtfeldman$elm_validate$Validate$validate, author$project$HomePage$ipValidator, newIp);
					if (_n1.$ === 'Err') {
						var errors = _n1.a;
						return errors;
					} else {
				...
				var newErrors = _Utils_update(
					oldErrors,
					{ip: newIpErrors});
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{errors: newErrors, ip: newIp}),
					elm$core$Platform$Cmd$none);
			case 'Submit':
				var _n2 = A2(rtfeldman$elm_validate$Validate$validate, author$project$HomePage$ipValidator, model.ip);
				if (_n2.$ === 'Err') {
					var errors = _n2.a;
					var oldErrors = model.errors;
					var newErrors = _Utils_update(
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{saveRequest: krisajenkins$remotedata$RemoteData$Loading}),
						author$project$HomePage$sendToServer(model));
				}
			default:
				var response = msg.a;
				return _Utils_Tuple2(
...
var author$project$HomePage$Submit = {$: 'Submit'};
var author$project$HomePage$UpdateIp = function (a) {
	return {$: 'UpdateIp', a: a};
};
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								elm$html$Html$Attributes$for('ip')
							]),
						_List_fromArray(
							[
								elm$html$Html$text('Ip')
							])),
						A2(
						elm$html$Html$input,
						_List_fromArray(
							[
								elm$html$Html$Attributes$name('ip'),
								elm$html$Html$Attributes$id('ip'),
								elm$html$Html$Attributes$value(model.ip),
								elm$html$Html$Events$onInput(author$project$HomePage$UpdateIp)
							]),
						_List_Nil),
						function () {
						var _n0 = model.errors.ip;
						if (!_n0.b) {
				...
				_List_fromArray(
					[
						elm$html$Html$Events$onClick(author$project$HomePage$Submit)
					]),
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						elm$html$Html$text('ip: ' + model.ip)
					])),
				A2(
				elm$html$Html$div,
				_List_Nil,
				_List_fromArray(
					[
						elm$html$Html$text('Submission result')
					])),
				A2(
				elm$html$Html$div,
				_List_Nil,
				_List_fromArray(
					[
						function () {
						var _n1 = model.saveRequest;
						switch (_n1.$) {
							case 'NotAsked':
								return elm$html$Html$text('Not yet requested!');
							case 'Loading':
								return elm$html$Html$text('In progress...');
							case 'Success':
